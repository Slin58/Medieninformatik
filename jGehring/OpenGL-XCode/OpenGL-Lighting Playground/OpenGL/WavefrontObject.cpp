...
vector<glm::vec3> WavefrontObject::triangleNormalVectors() {
    ...
    for(int j = 0; j < this->faceElements.size(); j++){
        int offset = 0 + (this->faceElements[j].x - 1) * 4;
        glm::vec3 vertice = glm::vec3(this->vertices[offset], this->vertices[offset+1], this->vertices[offset+2]);
        triangleVertices.push_back(vertice);
        this->faceElements[j].z = div(j, 3).quot + 1;
    }
    for(int i = 0; i < triangleVertices.size(); i+=3){
        glm::vec3 normal = glm::triangleNormal(triangleVertices[i], triangleVertices[i+1], triangleVertices[i+2]);
